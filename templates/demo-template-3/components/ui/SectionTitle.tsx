...
export function SectionTitle({
  ...
  const subtitleColorClass = variant === "white" ? "text-white/80" : "text-accent"

  return (
    <div className={cn("mb-8 md:mb-12", centered && "text-center", className)}>
      {subtitle && (
        <p className={cn("font-medium text-sm uppercase tracking-wider mb-3", subtitleColorClass)}>{subtitle}</p>
      )}
      <h2
        className={cn(
